...
def getPlainModel(d):
    ...
    advanced_act  = d.aact
    drop_prob     = d.dropout
    ch = 1 if d.model =='real' else 2
    inputShape    = d.input_shape
    ...
    if d.model == "real":
        ...
        O = Conv2D(sf, filsize, name='conv2', **convArgs)(O)
    else:
        # O = learnConcatRealImagBlock(I, (1, 1), (3, 3), 0, '0', convArgs, bnArgs, d)
        # O = Concatenate(channelAxis)([I, O])
        # ----------    unscaled data   ----------
        O = ComplexBN(name='bn_name_base', **bnArgs)(O)
        O = Activation(activation)(O)
        # ----------------------------------------
        O = ComplexConv2D(sf, filsize, name='conv1', **convArgs)(O)
        O = ComplexBN(name="bn_conv1_bn1", **bnArgs)(O)
        O = Activation(activation)(O)
        O = ComplexConv2D(sf, filsize, name='conv2', **convArgs)(O)
        O = ComplexBN(name="bn_conv2_bn2", **bnArgs)(O)
        O = Activation(activation)(O)
        O = ComplexConv2D(sf, filsize, name='conv3', **convArgs)(O)
        O = ComplexBN(name="bn_conv3_bn2", **bnArgs)(O)
        O = Activation(activation)(O)

        # O = ComplexConv2D(sf, filsize, name='conv4', **convArgs)(O)
        # O = ComplexBN(name="bn_conv4_bn2", **bnArgs)(O)
        # O = Activation(activation)(O)
        # O = ComplexConv2D(sf, filsize, name='conv5', **convArgs)(O)
        # O = ComplexBN(name="bn_conv5_bn2", **bnArgs)(O)
        # O = Activation(activation)(O)

        O = ComplexConv2D(sf, filsize, name='conv6', **convArgs)(O)
        O = ComplexBN(name="bn_conv2_bn3", **bnArgs)(O)
    ...
    if   dataset == ('cifar10' or "mnist"):
        O = Dense(128,  activation='relu')(O)
        O = Dense(10,  activation='softmax', kernel_regularizer=l2(0.0001))(O)
    elif dataset == 'others':
        # O = Dense(64,  activation='relu')(O)
        # O = ComplexBN(name="dense_bn1", **bnArgs)(O)
        # O = Dense(64,  activation='relu')(O)
        # O = Dense(128,  activation='relu')(O)
        # O = ComplexBN(name="dense_bn2", **bnArgs)(O)
        O = Dense(1, activation='sigmoid',kernel_regularizer=l2(0.0001))(O)
    else:
...
def getResnetModel(d):
    ...
    advanced_act  = d.aact
    drop_prob     = d.dropout
    #inputShape    = (3, 32, 32) if K.image_data_format() == "th" else (32, 32, 3)
    inputShape    = (256, 256, 3*2)
    channelAxis   = 1 if K.image_data_format() == 'channels_first' else -1
    filsize       = (3, 3)
    convArgs      = {
        "padding":                  "same",
...
#
# Print a newline after each epoch, because tensorflow.python.keras doesn't. Grumble.
#
...
class TestErrorCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        L.getLogger("train").info("Epoch {:5d} Evaluating on test set...".format(epoch+1))
        test_loss, test_acc = self.model.evaluate_generator(self.test_data, verbose=0)
        L.getLogger("train").info("                                      complete.")
...
class SaveLastModel(Callback):
    def __init__(self, workdir, period=10):
        ...
        self.chkptsdir        = os.path.join(self.workdir, "chkptsOriginal")
...
#
# Save record-bestOriginal models.
#
...
class SaveBestModel(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if val_acc > self.best_acc:
            ...
            # Print
            L.getLogger("train").info("Saved bestOriginal model to {:s} at epoch {:5d}".format(hdf5Filename, epoch+1))
...
def data_FFT(data):
    ...
    data_real = normalize(np.real(fft_data))
    data_imag = normalize(np.imag(fft_data))
    return np.concatenate([data_real, data_imag], axis = -1)
...
def train(d):
    ...
    img_path = '/home/zzzj/PycharmProjects/keras-complex/data/images_horse'
    test_path = '/home/zzzj/PycharmProjects/keras-complex/data/images_apple'

    # file_path = '/home/zzzj/PycharmProjects/keras-complex/data'
    # img_path = '/home/zzzj/PycharmProjects/keras-complex/data/images'
    # test_path = '/home/zzzj/PycharmProjects/pytorch-CycleGAN-and-pix2pix/results/apple2orange_pretrained/test_latest/images'

    partion = json.load(open(os.path.join(file_path, 'result.json')))
    labels = json.load(open(os.path.join(file_path, 'labels.json')))
    trainning_generator = DataGenerator(partion['train'], labels, data_root=img_path, batch_size=d.batch_size)
    val_generator       = DataGenerator(partion['val'],   labels, data_root=img_path, batch_size=d.batch_size)
    test_generator      = DataGenerator(partion['test'],  labels, data_root=test_path, batch_size=d.batch_size, mode='test')
    ...
    testErrCb      = TestErrorCallback(test_generator)
    ...
    model.fit_generator(generator = trainning_generator,
                        epochs    = d.num_epochs,
                        verbose=1,
                        callbacks=callbacks,
